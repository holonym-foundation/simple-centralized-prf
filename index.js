...
const _prf = (input) => {
	assert(
		typeof input === "bigint" && input < ORDER_r,
		`input must be a BigInt less than the bn254 prime ${ORDER_r}`,
	);
...
const authenticatedPRF = (preimage, digestFr) => {
	if (_verify(preimage, digestFr)) {
		const p = _prf(BigInt(digestFr));
...
/**
 *  @param {Object} req.body - the request body must be either a 
 *  { preimage, digestFr } object or an array of such objects.
 *  @returns {Object} - the PRF, the commit and the signature of the commitment with the digest.
    prfIn: the digestFr in the request
    prfOut: the prf
    bound: the commitment - a poseidon hash of the digestFr and the prf
    sig: the signature of the HOLONYM_SECRET_EDDSA and the commitment,
 */
...
app.post("/", async (req, res) => {
	...
	try {
		if (Array.isArray(req.body)) {
			res.send(
				req.body.map(({ preimage, digestFr }) =>
					authenticatedPRF(preimage, digestFr),
				),
			);
		} else {
			res.send(authenticatedPRF(req.body.preimage, req.body.digestFr));
		}
	} catch (error) {
		res.status(400);
	}
